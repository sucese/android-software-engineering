...
```
然后将该文件保存在项目资源中，例如，保存在 res/raw/keep.xml。构建不会将该文件打包到 APK 之中。上面提到可以用discard指定需要删除的资源，这里有人可能会疑惑，直接删了不就完
了，还要指定删除🤔。这个其实通常用在多构建应用变体之中，同一个应用可能包打包成不同的变体，不同变体需要的资源文件是不一样的，这样可以通过为不同变体定义不同的keep.xml来解决这
个问题。
